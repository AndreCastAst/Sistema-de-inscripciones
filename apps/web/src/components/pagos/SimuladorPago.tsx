...
<body>
  ...
  ${datos.tipo === "inscripcion"
    ...
    : datos.periodos?.map(p => `<div class="fila-item"><span>Cuota Ordinaria – ${formatPeriodo(p)}</span><span class="monto">S/ ${(1 / 1.18).toFixed(2)}</span></div>`).join("") ?? ""
  }
  <div class="linea"></div>
                    ...
                    {datos.tipo === "inscripcion" ? (
                      ...
                      datos.periodos?.map((p) => (
                        <div key={p} className="flex justify-between">
                          ...
                          <span className="text-on-surface font-semibold">S/ {(1 / 1.18).toFixed(2)}</span>
                        </div>
                      ))
                    )}
                  </div>
