  ...
  async function buscar() {
    ...
    try {
      ...
      if (data.estado !== "OBSERVADO") {
        setErrorBusqueda(
          data.estado === "PENDIENTE" || data.estado === "SUBSANADO"
    ...
    return (
      <>
        ...
        <main className="flex-grow w-full max-w-container-admin mx-auto px-md md:px-lg py-xl flex flex-col items-center justify-center gap-lg text-center">
          ...
          <h2 className="text-[28px] font-bold text-on-surface">¡Expediente reenviado!</h2>
          ...
          <button
            onClick={() => { setExito(false); setPostulacion(null); setDni(""); }}
            className="bg-primary text-on-primary px-xl py-sm rounded-lg text-[15px] font-semibold hover:brightness-110 transition-all"
          >
            Volver
          </button>
        </main>
      ...
      <main className="flex-grow w-full max-w-container-admin mx-auto px-md md:px-lg py-xl flex flex-col gap-xl">
        ...
        <section className="w-full max-w-[672px] mx-auto">
          <div className="bg-surface-container-lowest p-lg rounded-xl shadow-sm border border-outline-variant flex flex-col gap-md">
            <h1 className="text-[20px] font-semibold text-on-surface flex items-center gap-sm">
            ...
            <p className="text-[15px] text-on-surface-variant">
              Hemos detectado observaciones en su expediente. Por favor, corrija únicamente los campos/documentos señalados a continuación y reenvíe sin cargo adicional.
            </p>
          </div>
        </section>
        ...
        {!postulacion && (
          <section className="w-full max-w-[672px] mx-auto">
            <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-lg shadow-sm">
              <h2 className="text-[15px] font-semibold text-on-surface mb-md">Consultar Expediente por DNI</h2>
              <div className="flex gap-sm">
                <div className="relative flex-grow">
                  ...
                  <input
                    ...
                    maxLength={8}
                    className="w-full pl-xl pr-sm py-sm border border-outline-variant rounded-lg bg-surface focus:ring-2 focus:ring-primary focus:border-primary text-on-surface text-[15px] outline-none transition-all"
                  />
                </div>
                <button
                  type="button"
                  ...
                  className="flex items-center gap-xs px-lg py-sm bg-primary text-on-primary rounded-lg text-[15px] font-semibold hover:brightness-110 transition-all disabled:opacity-60"
                >
              ...
              {errorBusqueda && (
                <div className="mt-md bg-error-container border border-error/20 text-error rounded-lg px-md py-sm flex items-center gap-sm text-[15px]">
          ...
          <>
            ...
            <section className="w-full max-w-[672px] mx-auto">
              <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-lg shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-md">
                <div>
                  <p className="text-[13px] text-on-surface-variant mb-xs">Número de Expediente</p>
                  <p className="text-[15px] font-semibold text-on-surface mb-lg">
                    #CIP-{postulacion.creadoEn.slice(0, 4)}-{String(postulacion.id).padStart(4, "0")}
                  </p>
                  <div className="grid grid-cols-2 gap-lg">
                    <div>
                      <p className="text-[13px] text-on-surface-variant mb-xs">DNI</p>
                      <p className="text-[15px] font-semibold text-on-surface">{postulacion.dni}</p>
                    </div>
                    <div>
                      <p className="text-[13px] text-on-surface-variant mb-xs">Nombre Verificado</p>
                      <p className="text-[15px] font-semibold text-on-surface">
                        {postulacion.nombres} {postulacion.apellidoPaterno} {postulacion.apellidoMaterno}
                      </p>
                    </div>
                  </div>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-[13px] text-on-surface-variant mb-xs">Estado Actual</p>
                  <span className="inline-block px-md py-sm bg-status-observado-bg text-status-observado-text rounded-full text-[13px] font-bold">
                    Observado
                  </span>
                  <button
                    onClick={() => { setPostulacion(null); setDni(""); }}
                    className="block mt-sm text-[13px] text-primary hover:underline"
                  >
                    Cambiar
                  </button>
                </div>
                    ...
                    <div className="bg-status-observado-bg border border-status-observado-text/20 rounded-lg p-md">
                      ...
                      <ul className="space-y-xs">
                        {postulacion.observaciones.slice(0, 3).map((obs) => (
                          <li key={obs.id} className="text-[13px] text-on-surface-variant">• {obs.mensaje}</li>
                        ))}
                      </ul>
                    </div>
                  )}
            ...
            <section className="w-full max-w-[672px] mx-auto">
              <div className="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm">
                ...
                <div className="p-lg grid grid-cols-1 md:grid-cols-2 gap-lg">
                  ...
                  <div>
                    <label className="block text-[13px] font-medium text-on-surface-variant mb-xs">Carrera Asignada</label>
                    <input
                      readOnly
                      value={postulacion.carrera ? `Ing. ${postulacion.carrera.nombre}` : "Por asignar"}
                      className="w-full rounded-lg border border-outline-variant bg-surface-container-low px-md py-sm text-on-surface-variant text-[15px] cursor-not-allowed outline-none"
                    />
            ...
            <section className="w-full max-w-[672px] mx-auto">
              <div className="bg-status-aprobado-bg border border-status-aprobado-text/20 rounded-xl p-md flex items-start gap-md">
                ...
                <div>
                  <h3 className="text-[15px] font-semibold text-status-aprobado-text mb-xs">Sin Cobro Adicional</h3>
                  <p className="text-[15px] text-on-surface-variant">
                    Esta subsanación no genera cobros adicionales. Su cuota de inscripción (S/ 3.00) ya ha sido registrada.
                  </p>
                  <p className="text-[13px] text-on-surface-variant mt-sm">
                    <span className="font-semibold text-on-surface">Intentos sin límite:</span>{" "}
                    Puede reenviar su expediente cuantas veces sea necesario hasta que sea aprobado.
                  </p>
                </div>
              </div>
            </section>
            ...
            <section className="w-full max-w-[672px] mx-auto flex flex-col gap-md">
              ...
              <p className="text-[13px] text-on-surface-variant text-center">
                Al reenviar, el sistema cambiará automáticamente el estado del expediente a &quot;Subsanado&quot; y lo presentará nuevamente al Revisor Regional.
              </p>
            </section>
          </>
        )}
