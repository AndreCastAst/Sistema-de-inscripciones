...
import { buscarColegiado, obtenerEstadoCuenta, api, type EstadoCuenta } from "@/lib/api";
...
function PanelDeudaYPago({
  ...
  async function confirmarVoucher() {
    ...
    setErrorPago(null);
    try {
      for (const m of cuotasPendientes) {
  ...
  if (exitoPago) {
    return (
      <div className="bg-status-aprobado-bg border border-status-aprobado-text/20 rounded-xl p-lg flex items-center gap-md">
        ...
        <div>
          <h3 className="font-body-bold text-body-bold text-status-aprobado-text">¡Pago registrado!</h3>
    ...
    <div className="flex flex-col gap-lg w-full">
      ...
      <div className={`rounded-xl p-md border ${
        ...
        <div className="flex items-start gap-md mb-md">
          ...
          <div>
            <h3 className={`font-body-bold text-body-bold ${inhabilitado ? "text-status-rechazado-text" : "text-status-aprobado-text"}`}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-sm mt-xs">
          <div className={`rounded-lg px-md py-sm ${inhabilitado ? "bg-white/50" : "bg-white/60"}`}>
            <p className="text-[12px] text-on-surface-variant">Deuda total</p>
            <p className={`text-[17px] font-bold ${inhabilitado ? "text-status-rechazado-text" : "text-status-aprobado-text"}`}>
              S/ {(cuenta?.totalDeuda ?? 0).toFixed(2)}
            </p>
          </div>
          <div className={`rounded-lg px-md py-sm ${inhabilitado ? "bg-white/50" : "bg-white/60"}`}>
            <p className="text-[12px] text-on-surface-variant">Cuotas pendientes</p>
            <p className={`text-[17px] font-bold ${inhabilitado ? "text-status-rechazado-text" : "text-status-aprobado-text"}`}>
              {cuotasPendientes.length} {cuotasPendientes.length === 1 ? "mes" : "meses"}
            </p>
          </div>
          <div className={`rounded-lg px-md py-sm col-span-2 sm:col-span-1 ${inhabilitado ? "bg-white/50" : "bg-white/60"}`}>
            ...
            <p className={`text-[15px] font-semibold ${inhabilitado ? "text-status-rechazado-text" : "text-status-aprobado-text"}`}>
              {inhabilitado
                ? `Vencido desde ${cuotasPendientes[0] ? formatPeriodo(cuotasPendientes[cuotasPendientes.length - 1].periodo) : "—"}`
                : "Vigente y válido"}
            </p>
          </div>
        </div>
      ...
      {inhabilitado && cuotasPendientes.length > 0 && (
        <div className="bg-surface-container-lowest rounded-xl border border-outline-variant overflow-hidden shadow-sm">
          <div className="bg-surface border-b border-outline-variant px-lg py-md flex justify-between items-center">
            <h3 className="font-headline-md text-headline-md text-on-surface">Detalle de Deuda</h3>
            <span className="font-label-sm text-label-sm text-on-surface-variant bg-surface-container px-sm py-xs rounded border border-outline-variant">
              Cuota Mensual: S/ 20.00
            </span>
          </div>
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-outline-variant bg-surface-container-low">
                <th className="font-label-sm text-label-sm text-on-surface-variant px-lg py-sm">Concepto</th>
                <th className="font-label-sm text-label-sm text-on-surface-variant px-lg py-sm text-right">Monto</th>
              </tr>
            </thead>
            <tbody className="font-body-base text-body-base text-on-surface">
              ...
              <tr className="bg-surface-dim">
                <td className="px-lg py-md font-headline-md text-headline-md text-right">Total a Pagar</td>
                <td className="px-lg py-md font-headline-md text-headline-md text-right text-primary tabular-nums">
          ...
          <div className="p-lg bg-surface border-t border-outline-variant flex flex-col gap-md">
            <h4 className="font-body-bold text-body-bold text-on-surface">Registrar Pago</h4>
      ...
      <main className="flex-grow w-full max-w-container-admin mx-auto px-md md:px-lg py-xl flex flex-col gap-xl">
        ...
        <section className="w-full max-w-container-max-form mx-auto">
          <div className="bg-surface-container-lowest p-lg rounded-xl shadow-sm border border-outline-variant flex flex-col gap-md">
            <h1 className="font-headline-md text-headline-md text-on-surface">Consulta de Estado y Habilidad</h1>
            ...
            <div className="flex flex-col sm:flex-row gap-sm mt-sm">
              <div className="relative flex-grow">
                ...
                <input
                  ...
                  className="w-full h-[48px] pl-[48px] pr-md bg-surface-bright border border-outline-variant rounded focus:border-primary focus:border-2 outline-none transition-all font-body-base text-body-base text-on-surface placeholder:text-secondary-fixed-dim"
                />
              </div>
              <button
                onClick={buscar}
                disabled={buscando || !query.trim()}
                className="h-[48px] px-xl rounded flex items-center justify-center gap-sm font-body-bold text-body-bold bg-primary text-on-primary hover:brightness-110 transition-all shrink-0 disabled:opacity-60"
              >
                {buscando && <Spinner />}
                Buscar
              </button>
        ...
        {carnet && (
          <section className="grid grid-cols-1 lg:grid-cols-[auto_1fr] gap-xl items-start">
            ...
            <div className="flex flex-col gap-sm items-center lg:items-start w-full lg:w-auto">
              <h2 className="font-body-bold text-body-bold text-on-surface px-sm">Vista Previa de Carnet</h2>
